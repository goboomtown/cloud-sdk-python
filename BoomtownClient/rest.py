...
class RESTClientObject(object):
    ...
    def request(self, method, url, query_params=None, headers=None,
        ...
        headers.update(self.sign_request(url, query_params))
    ...
    def sign_request(self, url, query_params=None):

        time = datetime.datetime.utcnow().isoformat()
        query = "?" + urlencode(query_params) if query_params else ""
        token = Configuration().api_key['token']
        secret = Configuration().api_key['secret']
        resource = urlparse(url).path + query + ":" + time
        print resource

        auth = {}
        if token and secret:
            key = bytes(secret).encode('utf-8')
            uri = bytes(resource).encode('utf-8')
            auth['X-Boomtown-Date'] = time
            auth['X-Boomtown-Token'] = token
            auth['X-Boomtown-Signature'] = base64.b64encode(hmac.new(key, uri, digestmod=hashlib.sha256).digest())
